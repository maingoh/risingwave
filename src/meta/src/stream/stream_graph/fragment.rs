...
pub fn check_sink_fragments_support_refresh_schema(
    ...
    // Delegate operator support/invariant validation to the framework rules.
    DEFAULT_SINK_SCHEMA_REWRITER.validate_root(sink_node)?;
    Ok(())
}
...
pub fn rewrite_refresh_schema_sink_fragment(
    ...
    DEFAULT_SINK_SCHEMA_REWRITER.rewrite_root(&mut new_sink_fragment.nodes, &change_set, &ctx)?;

    let sink_node = &new_sink_fragment.nodes;
